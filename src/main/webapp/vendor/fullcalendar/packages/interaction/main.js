...
(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var extendStatics = function(dExtended, bExtended) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(dExtended, bExtended);
    };
    ...
    */
    var OffsetTracker = /** @class */ (function () {
        function OffsetTracker(elOffSetTracker) {
            this.origRect = core.computeRect(elOffSetTracker);
            // will work fine for divs that have overflow:hidden
            this.scrollCaches = core.getClippingParents(elOffSetTracker).map(function (el) {
    ...
    var HitDragging = /** @class */ (function () {
        function HitDragging(dragging, droppableStore) {
            ...
            this.handlePointerDown = function (ev) {
                var localDragging = _this.dragging;
                ...
                if (_this.initialHit || !_this.requireInitial) {
                    localDragging.setIgnoreMove(false);
                    _this.emitter.trigger('pointerdown', ev); // TODO: fire this before computing processFirstCoord, so listeners can cancel. this gets fired by almost every handler :(
                }
                else {
                    localDragging.setIgnoreMove(true);
                }
            };
    ...
    var DateClicking = /** @class */ (function (_super) {
        ...
        function DateClicking(settings) {
            ...
            _this.handleDragEnd = function (ev) {
                var localComponent = _this.component;
                var pointer = _this.dragging.pointer;
                if (!pointer.wasTouchScroll) {
                    var _a = _this.hitDragging, initialHit = _a.initialHit, finalHit = _a.finalHit;
                    if (initialHit && finalHit && isHitsEqual(initialHit, finalHit)) {
                        localComponent.calendar.triggerDateClick(initialHit.dateSpan, initialHit.dayEl, localComponent.view, ev.origEvent);
                    }
                }
            };
    ...
    */
    var DateSelecting = /** @class */ (function (_super) {
        ...
        function DateSelecting(settings) {
            ...
            _this.handlePointerDown = function (ev) {
                var _a = _this, handleComponent = _a.component, handleDragging = _a.dragging;
                var canSelect = handleComponent.opt('selectable') &&
                    handleComponent.isValidDateDownEl(ev.origEvent.target);
                // don't bother to watch expensive moves if component won't do selection
                handleDragging.setIgnoreMove(!canSelect);
                // if touch, require user to hold down
                handleDragging.delay = ev.isTouch ? getComponentTouchDelay(handleComponent) : null;
            };
    ...
    var EventDragging = /** @class */ (function (_super) {
        ...
        function EventDragging(settings) {
            ...
            _this.handlePointerDown = function (ev) {
                ...
                var _a = _this, handlePointerDownComponent = _a.component, handlePointerDowndragging = _a.dragging;
                var mirror = handlePointerDowndragging.mirror;
                var initialCalendar = handlePointerDownComponent.calendar;
                var subjectSeg = _this.subjectSeg = core.getElSeg(ev.subjectEl);
                var eventRange = _this.eventRange = subjectSeg.eventRange;
                var eventInstanceId = eventRange.instance.instanceId;
                _this.relevantEvents = core.getRelevantEvents(initialCalendar.state.eventStore, eventInstanceId);
                handlePointerDowndragging.minDistance = ev.isTouch ? 0 : handlePointerDownComponent.opt('eventDragMinDistance');
                handlePointerDowndragging.delay =
                    // only do a touch delay if touch and this event hasn't been selected yet
                    (ev.isTouch && eventInstanceId !== handlePointerDownComponent.props.eventSelection) ?
                        getComponentTouchDelay$1(handlePointerDownComponent) :
                        null;
                mirror.parentNode = initialCalendar.el;
                mirror.revertDuration = handlePointerDownComponent.opt('dragRevertDuration');
                var isValid = handlePointerDownComponent.isValidSegDownEl(origTarget) &&
                    !core.elementClosest(origTarget, '.fc-resizer'); // NOT on a resizer
                handlePointerDowndragging.setIgnoreMove(!isValid);
    ...
    var EventDragging$1 = /** @class */ (function (_super) {
        ...
        function EventDragging(settings) {
            ...
            _this.mutatedRelevantEvents = null;
            _this.handlePointerDown = function (ev) {
                var handlePonterDownComponent = _this.component;
                var seg = _this.querySeg(ev);
                var eventRange = _this.eventRange = seg.eventRange;
                _this.dragging.minDistance = handlePonterDownComponent.opt('eventDragMinDistance');
    ...
    var ExternalElementDragging = /** @class */ (function () {
        function ExternalElementDragging(dragging, suppliedDragMeta) {
            ...
            this.handleHitUpdate = function (hit, isFinal, ev) {
                var handleHitDragging = _this.hitDragging.dragging;
                var receivingCalendar = null;
                ...
                // TODO: wish we could somehow wait for dispatch to guarantee render
                handleHitDragging.setMirrorIsVisible(isFinal || !droppableEvent || !document.querySelector('.fc-mirror'));
                if (!isInvalid) {
                    core.enableCursor();
                }
                else {
                ...
                if (!isFinal) {
                    handleHitDragging.setMirrorNeedsRevert(!droppableEvent);
                    _this.receivingCalendar = receivingCalendar;
    ...
    */
    var ExternalDraggable = /** @class */ (function () {
        function ExternalDraggable(el, settings) {
            ...
            this.handlePointerDown = function (ev) {
                var handlePoninterDownDragging = _this.dragging;
                var _a = _this.settings, minDistance = _a.minDistance, longPressDelay = _a.longPressDelay;
                handlePoninterDownDragging.minDistance =
                    minDistance != null ?
                        minDistance :
                        (ev.isTouch ? 0 : core.globalDefaults.eventDragMinDistance);
                handlePoninterDownDragging.delay =
